...
const dataEndpoint = (router) => {
    ...
    router.post('/api/posts', async (request, response, next) => {posts.push(request.body.newPost);
        response.status(200).send({post: posts[posts.length - 1]});
    });
    router.get('/api/post/:id', async (request, response, next) => {
        console.log(request.params.id);
        response.status(200).send({get: posts[request.params.id]});
    });
    router.put('/api/post/:id', async (request, response, next) => {
        if(request.body.updatePost.title != null) posts[request.params.id].title = request.body.updatePost.title
        if(request.body.updatePost.text != null) posts[request.params.id].text = request.body.updatePost.text
        if(request.body.updatePost.image != null) posts[request.params.id].image = request.body.updatePost.image
        response.status(200).send({ put: posts[request.params.id] });
    })
